...
def find_way(field, path, visited, possible_paths, memory):
    x, y = path[-1]
    print_map(field, (x, y), visited)
    sleep(0.02)

    if possible_paths and len(path) >= len(possible_paths[-1]):
        return

    if (x, y) in memory and len(path) >= memory[(x, y)] and USE_MEMORY:
        return
    memory[(x, y)] = len(path)
    ...
    for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
        ...
        if (nx, ny) not in visited and field[ny][nx] == " ":
            find_way(field, path + [(nx, ny)], visited + [(nx, ny)], possible_paths,memory)
